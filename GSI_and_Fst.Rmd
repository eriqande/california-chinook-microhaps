...
No locus has more than 9 alleles, so we can do single-digits for alleles.  We drop the
LFAR from this, because those were chosen specifically to be weird in the late fall.
```{r}
hfs_ref_long <- full_base2 %>% 
  select(-starts_with("NC_"), -starts_with("Ots")) %>%  # see what we get when we kep just the tag_id markers
  mutate(
  ...
  group_by(locus) %>%
  mutate(
    alle_int =  as.integer(factor(value)),
    alle_int = ifelse(is.na(alle_int), 0L, alle_int)
  ) %>%
  ungroup() %>%
  select(-value)
...
hfs_ref_wide <- hfs_ref_long %>%
  ...
  ungroup() %>%
  mutate(geno = ifelse(geno == "00", NA_character_, geno)) %>%
...
Something is very, very weird here.  Look at that Fst matrix:
```{r}
tmp <- fst_mat
rownames(tmp) <- collection_order
tmp
```

For some reason, BCF, BCS, MDS, and BCF have quite low Fst values to EVERYONE, even Winter run and the
Klamath.  That just cannot be right!!  They have super low Fst values to FRHF and FRHF, but those
two pops have Fst of around 0.14 to winter-run, and BCS to winter run is only 0.0269.  WTF?! 
