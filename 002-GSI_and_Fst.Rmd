...
which is only about 150 of them, so that should be fine. It will make downstream processing
...
This is interesting.  I am struck by the 12 instances of EEHEEEEE---that suggests to me that
it is not simply the result of genotyping error.  I suspect that one of the haplotypes
has an L at that position.  The cleanest thing here might be to simply go for the
things that are perfectly pure across all the sites, cuz there is a much lower chance
of the region being broken up by recombination or gene conversion there.  So, that is what
I am going to do.  And we can look at the ones that are not monolithic (possible recombinants, etc.
as well.)
