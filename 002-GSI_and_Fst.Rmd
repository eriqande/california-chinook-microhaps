...
# Adding genic differentiation values to lower triangle and making a figure

We are going to use genepop to compute p-values for differentiation.  To do
that we need to make a genepop file.
```{r}
gpt <- full_base2 %>%
...
Now, we could use the genepop package to get results from that file, but it
is crazy slow, so we do not do this.
```r
td_file <- "results/GSI_and_Fst/genic_tests.txt"
genepop::test_diff(inputFile = outf, pairs = TRUE, outputFile = td_file)
```

OK! HERE IS WHAT WE WILL DO: MAKE THIS PARALLELIZABLE.  WE WILL MODIFY THE ABOVE
TO MAKE A GENEPOP FILE FOR EVERY LOCUS, AND THEN WE WILL JUST RUN OVER ALL THOSE
LOCI USING mclappy ON 8 CORES ON MY LAPTOP, TO MAKE IT FASTER.  THEN, WE WILL PRESENT
THE RESULTS AS A TINY-MULTIPLE HISTOGRAM OF THE P-VALUES IN EACH CELL.  KINDA COOL!
