...
no_lfar_ass_table <- plot_self_assignment_table(top_ass, pop_labels, collection_order)

no_lfar_ass_table_80 <- top_ass %>% 
  filter(scaled_likelihood > 0.8) %>%
  plot_self_assignment_table(pop_labels, collection_order)

no_lfar_ass_table_Rosa <- top_ass %>%
...
lfar_ass_table <- plot_self_assignment_table(lfar_top_ass, pop_labels, collection_order)

lfar_ass_table_80 <- lfar_top_ass %>% 
  filter(scaled_likelihood > 0.8) %>%
  plot_self_assignment_table(pop_labels, collection_order)

lfar_ass_table_Rosa <- lfar_top_ass %>%
  left_join(rosa_genos, by = join_by(indiv)) %>%
  plot_self_assignment_table(pop_labels, collection_order)
```
...
repu_likes <- full_plus_lfar_sa %>%
  group_by(indiv, collection, repunit, inferred_repunit) %>%
  summarise(repu_sclike = sum(scaled_likelihood)) %>%
  ungroup()
...
```{r}
misassigned_ones <- top_ass_w_lfar %>%
  filter(repunit != inferred_repunit) %>%
