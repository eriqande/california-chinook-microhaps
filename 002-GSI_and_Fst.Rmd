...
if(exists("snakemake")) {
  ...
  input_list <- list(
    ...
    final_baseline_apr = "data/subRoSA_baseline_rubias_APR2024_w_mixture_revised_rosa_added.csv",
...
full_base_apr <- read_csv(input_list$final_baseline_apr) %>%
  filter(collection != "SacNonW")

# remove the rosa hapstr from it
full_base <- full_base_apr %>%
  select(-hapstr)

# capture that rosa hapstr with some meta data from future use
rosa_hapstrs <- full_base_apr %>%
  select(indiv:hapstr)
...
# Include the LFAR in there in a version
...
We are not going to delve on the different between E1 and E2 here.  That is for the
...
H8 <- rosa_hapstrs %>%
  select(indiv, hapstr) %>%
  mutate(
    hvec = map(hapstr, function(x) {y <- strsplit(x, "")[[1]]; y[c(1, 4, 5, 7:11)]}),
...
From that it is clear that there have been some recombinations in here, but we will
just go ahead and call RoSA genotype by the majority of the non-missing ones. 
```{r}
H8_strip <- H8 %>%
  mutate(
    stripped = map(hvec, function(x) x[!(x %in% c("?", "N"))]),
    sglom = map_chr(stripped, function(x) paste(x, collapse = "", sep = ""))
    ) 
...
OK, we will call something and EL if it has two or more Hs (the idea being that a single
H could be a genotyping error). Note that there are no haplotypes mixing L's and E's together.
Here is a function that will determine the RoSA genotype
from a vector of H, L, and E parts. 
```{r}
call_rosa <- function(x) {
  # things that were all ?'s get NA
  if(length(x) == 0) {
...
H8_genos <- H8_strip %>%
  mutate(rosa_geno = map_chr(stripped, call_rosa))
```

Count these up so we can see what we called LL, EL, and EE:
```{r}
H8_genos %>%
  count(sglom, rosa_geno) %>%
  arrange(desc(n))
```
...
collection within that repunit.  We include the lfar in here.
...
```{r}
lfar_sa <- self_assign(base_with_lfar, gen_start_col = 5)
...
```{r}
over80 <- nrow(lfar_top_ass$top %>% filter(repu_sclike > 0.8))

lfar_top_ass$top %>% filter(repu_sclike > 0.8) %>% count(repunit == inferred_repunit) %>% mutate(fract = n / over80)
```
...
## Identifying fish erroneously included in some collections

The occurrence of winter-run-assigned fish amongst the spring run collections is
a clear indicator that there are some fish in the collections that do not belong
with one another.  The same occurs amongst the fall and the spring run, I expect.

Our goal here is to examine the likelihoods with which fish are assigned to
different reporting units and come up with a common-sense and defensible approach
for discarding individuals that don't belong in the collections they were sampled
from.  

We start by tallying up the misassigned fish---those that have a repunit and an
inferred repunit that differ:
```{r}
misassigned_ones <- lfar_top_ass$top %>%
...
For each of these 6 pairs, I want to make a scatterplot that shows the repunit
likelihoods of all the fish _from either the repunit or the inferred_repunit_.
The x-axis will be the likelihood for the repunit and the y-axis will be the
likelihood for the inferred_repunit.  So, there is some weird prep to get
this all together.  
```{r}
MO2 <- misassigned_ones %>%
  rename(
    Foc_repunit = repunit,
    Foc_inferred_repunit = inferred_repunit
  )


pick_em <- lapply(1:nrow(MO2), function(i) {
  
  FR <- MO2$Foc_repunit[i]
  FIR <- MO2$Foc_inferred_repunit[i]
  
  lfar_top_ass$repu_likes %>%
    ungroup() %>%
    filter(
      (repunit == FR & inferred_repunit == FIR) |
        (inferred_repunit == FR & repunit == FIR)
    ) %>%
    mutate(
      Foc_repunit = FR,
      Foc_inferred_repunit = FIR,
      .before = indiv
    ) 
}) %>%
    bind_rows()
...
```{r}
tmp <- lapply(1:nrow(MO2), function(i) {
  lfar_top_ass$repu_likes %>%
    filter(
      (repunit == MO2$Foc_repunit[i] | repunit == MO2$Foc_inferred_repunit[i]) &
        (inferred_repunit == MO2$Foc_repunit[i] | inferred_repunit == MO2$Foc_inferred_repunit[i])
    ) %>%
    mutate(
      Foc_repunit =  MO2$Foc_repunit[i],
      Foc_inferred_repunit = MO2$Foc_inferred_repunit[i],
      .before = repunit
    )
}) %>%
  bind_rows() %>%
  arrange(indiv, Foc_repunit, Foc_inferred_repunit)


for_plots <- tmp %>%
  mutate(
    rep = ifelse(Foc_repunit == repunit, "rep", "inf"),
    inf = ifelse(Foc_inferred_repunit == inferred_repunit, "inf", "rep")
  ) %>%
  ungroup() %>%
  #count(Foc_repunit, Foc_inferred_repunit, repunit, inferred_repunit, rep, inf)
  select(-repunit, -inferred_repunit) %>%
  pivot_wider(names_from = c(rep, inf), names_sep = "_to_", values_from = repu_sclike) %>%
  mutate(condition = str_c(Foc_repunit, " misassigned to ", Foc_inferred_repunit), .after = Foc_inferred_repunit)
```


Now we should be able to facet wrap those:
```{r}
ggplot(for_plots) +
  geom_point(aes(x = inf_to_rep, y = inf_to_inf, colour = Foc_inferred_repunit)) +
  geom_point(aes(x = rep_to_rep, y = rep_to_inf, colour = Foc_repunit)) +
  facet_wrap(~condition) +
  scale_colour_manual(values = repunit_colors)

```


So, for each of the six plots, I will need four different repu_sclike's:
For example, for the `repunit = CV-Fall` and `inferred_repunit = CV-Spring` case
we will need to have the repu_sclikes for CV-Fall fish being calculated for CV-Fall
and CV-Spring, and for the CV-Spring fish being calculated for CV-Spring and CV-Fall.


Now, we want to use this to semi-join to get all the likelihoods for these scenarios,
whether or not they were misassigned. 
```{r}
focal_comparisons <- repu_likes %>%
  semi_join(misassigned_ones, by = join_by(repunit, inferred_repunit))
```

And we are going to want to compare these to all the likelihoods of being
from the actual repunit that the fish was col


Now, I want to look at these different comparisons _pairwise_, so we will
just pivot it wider and then look at different columns in there.  This isn't
super "tidy" but I think it should work OK:
```{r}
focal_comparisons %>%
  pivot_wider(names_from = inferred_repunit, values_from = repu_sclike)
```

And I want to look at the observed distribution of these scores in for each
comparison, colored by repunit.
```{r}
ggplot(focal_comparisons, aes(x = ))
```
       
       
       
       
First thing I am going to do is plot histograms of the,
max likelihoods for all of the ones from each repunit.
```{r}
repunit_order <- unique(pop_labels$repunit)
repu_likes %>%
  mutate(
    repunit_f = factor(repunit, levels = repunit_order),
    inferred_repunit_f = factor(inferred_repunit, levels = repunit_order),
  ) %>%
  ggplot() +
  geom_histogram(aes(x = repu_sclike, fill = repunit_f), colour = "black", linewidth = 0.16) + 
  facet_grid(repunit_f ~ inferred_repunit_f) + 
