...
```{r}
# get just the 2 LFAR markers we are using.  We are dropping 828619 because
# it really doesn't do much, and so many reads are off-target.
lfar_columns <- paste(
  rep(c(
...
## Follow up on those Early alleles found in RRF and ERF

The analysis shows 4 fish from RRF and ERF with heterozygous genoypes (EL).
This has not been previously reported, so we really want to check up on those.
It might be that they had lots of missing data.  

So, I want to grab their RoSA genotype strings so that we can verify.
```{r}
# here are the IDs fo the hets from those coastal pops
coastal_hets <- lfar_top_ass$top %>%
  left_join(rosa_genos, by = join_by(indiv)) %>%
  filter(collection %in% c("ERF", "RRF")) %>%
  select(indiv, rosa_geno) %>%
  filter(rosa_geno %in% c("EE", "EL"))
coastal_hets
```

Now, let's get the raw genotype strings (the "hapstrs") off those guys:
```{r}
rosa_hapstrs %>%
  semi_join(coastal_hets, by = join_by(indiv))
```

Those are definitely hets in the left half of the markers.  One of them is also
hets at the right half.  The three remaining ones are L homozygotes on the second
half.  So, there was been some recombination within these.  
...
exploded <- rosa_hapstrs %>%
  filter(!(collection %in% c("ColeRHsp", "SmithFall"))) %>%
  separate(hapstr, into = paste("s", 0:12, sep = ""), sep = "") %>%
  select(-s0)
rh_counts <-  exploded %>%
...
So, there is only one more E2-het outside of the Sacto, and that is from TRHsp.

Let's go back and find that:
```{r}
exploded %>% 
  filter(repunit == "klamathspring" & s6 == "H") %>%
  left_join(rosa_hapstrs, by = join_by(indiv))
