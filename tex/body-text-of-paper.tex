...
less productive stocks \comment{(CITE)}. The co-occurence of fish from relatively productive and relict populations reaches its paragon in California,
...
same river basin as the highly endangered and phenotypically distinct winter-run stock and the threatened spring-run stocks \comment{(CITE)}.
...
harvested fall-run stock \comment{(CITE)}.
...
spring-run and fall-run ecotypes (i.e. Mill, Deer and Butte Creeks), previous research has documented different  (but overlapping) ranges of spawning time (Julian date) for each ecotype \comment{(CITE)}.  As such, sampling targeted carcasses found in each of these
...
\comment{Gotta add the WRAP discovery and filtering stuff.}
...
unit are colored according to the reporting unit.  In ({\bf a}), for $F_\mathrm{ST}$ values,
...
The program STRUCTURE clustered fish at all values of $K$ from 2 to 7.
...
grouped together into reporting units.  At every value of $K$ CLUMPAK discerned \comment{at least
11 of 20 replicates} in the major mode (Fig.~\ref{fig:struct}).   
\comment{Need to add more description of the patterns, especially at $K=7$.}
\comment{Clustering solutions in the minor
