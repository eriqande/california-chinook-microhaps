...
Of the 1594 total fish samples from 17 different collections, 724 fish from 10 collections were of the fall run ecotype,
...
we first conducted a simple case-control genome-wide association study (GWAS) using the low-coverage whole-genome sequencing
...
We surveyed the GWAS $p$-values relative to the commonly used significance threshold for
...
that were adjacent to, but not directly within, the main peak of association, as we had hoped that those SNPs
might enable us to learn about recombination in the region.  
...
Methods, yielded 49 candidate markers for which we designed primers for amplication (Figure~\ref{fig:lfar-candi}).  Only 8
...
winter run from all other groups of Chinook salmon in the CCV. Initial examination of sequence data 
in the 16 winter-run fish and 84 non-winter run fish revealed
a number of SNPs that were potentially diagnostic, but none were fixed for alternative alleles 
in a larger sample.
Nonetheless, three of these markers from that effort had large frequency differences
