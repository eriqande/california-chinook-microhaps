...
using simple operations in the R programming language, Version 4.2
\citep{rcore}. We start with a simple tally of the number of alleles
found at each locus.  To provide a summary of the genetic variation and
missing data we report 1) the average over loci of the number of fish
in which each locus was successfully genotyped ($\bar{N}$);
2) the average number of alleles per locus with all collections
downsampled to the smallest number of genotyped individuals at each
locus ($\bar{N}_{A,ss}$);  3) the fraction of polymorphic
loci in each collection after subsampling to the smallest number of genotyped
fish ($\bar{P}_{poly,ss}$); 4) the expected heterozygosity (average over loci
of one minus the frequencies of homozygotes at each locus expected from the
estimated allele frequencies; and 5) the observed heterozygosity (average over
loci of the fraction of heterozygous genotypes.
...
structure in the data by performing unsupervised clustering of the
the complete data set (\comment{With or without the LFAR? Which did Anthony use?}) 
...
using default settings of the program. 
...
showed more than two alleles, providing significantly more power for relationship
inference than a comparably sized panel of biallelic SNPs.
