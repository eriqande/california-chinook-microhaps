...
in freshwater every month of the year in California \citep{fisher1994past}.  Our previous work \citep{thompson2020complex} documented
a strong association between these ecotypes in the CCV and the alleles carried near the GREB1L and ROCK1 genes
in a section of chromosome 28 that we termed the region of strongest association (RoSA), a pattern
also observed in other river basins of the West Coast \citep[summarized in][]{waples2022implications}.  
...
from 17 collections, genotyped at 204 amplicon sequences \citep{baetscher2018microhaplotypes} from throughout the genome.
This baseline is
...
This situation carries the potential for untagged spring run 
phenotypes to be included as fall-run spawners if they were not 
encountered during the spring trapping season.
 We sampled putative spring-run and fall-run fish 
...
assessment of Hardy-Weinberg equilibrium \citep{hardy1908mendelian} was conducted in MICROHAPLOT (Ng {\em et al.}, DOI: 10.5281/zenodo.820110).  Loci that produced more than two haplotypes within an individual, as well as those clearly violating Hardy-Weinberg equilibrium within populations were removed.
...
We include a novel amplicon that targets the sex-determining region ({\em sdY}) on the Y chromosome in Chinook salmon. We designed this amplicon to target the SNP described by \citet{bertho2022nonfunctional} that differentiates the functional {\em sdY} and a nonfunctional {\em sdY} pseudogene.  Examination of whole-genome sequence data in multiple populations of California Chinook salmon \citep{thompson2020complex} shows that this
marker characterizes genetic sex in diverse California salmon populations more accurately than previously published \citep{von2015development} marker assays.
And, finally, we include marker Ots\_coho001\_05\_32691399 which is fixed for alternate alleles in
coho salmon and Chinook salmon.  This is helpful when samplers have unwittingly
...
Libraries were originally quantified by qPCR with the NEBNext Library Quant Kit for Illumina (New
England BioLabs); however more recently we have quanitifed our libraries
...
(for the LFAR, WRAP, VGLL3, and Six6 markers) using BWA mem\citep{bwa-mem2009}. Mapped reads were converted to BAM files using SAMtools \citep{li2009sequence} and FreeBayes \citep{garrison2012haplotype} was used to call variants.
...
the RoSA.  For this we identified the RoSA genotype \citep[EE, EL, or LL, see~][]{thompson2020complex} from the concordance of the genotypes
...
winter run from all other groups of Chinook salmon in the CCV.  We found
a number of SNPs that were fixed for alternate alleles between the 16 winter-run fish
and 84 non-winter run fish that we had sequenced; however after designing assays for many of these
SNPs and testing them in a much larger sample, we did not find any truly diagnostic loci.
Nonetheless three markers from that effort are included in
...
the extreme bottleneck that it passed through in the 1980's and 1990's \citep{hedrick1994effective}. The inclusion 
...
E-lineage haplotypes, in either heterozygous or homozygous form, which are then positively selected
...
Jeff Rodzen. USDA is an equal opportunity provider and employer. Mention of trade names
