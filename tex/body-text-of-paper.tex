...
management, having largely replaced coded wire tags in large parts of the Snake River and
Columbia River basin (CITATION), and having been employed to dramatically further our
understanding of the genetic inheritance of key traits in salmonids
\citep{abadia2013large,beulke2023distinct}. However, PBT is just one subset of whole
...
California's Central Valley, the Klamath and Trinity River basins in Northern California, and
...
spring-run and fall-run ecotypes (i.e. Mill, Deer and Butte Creeks), previous research has documented different  (but overlapping) ranges of spawning time (Julian date) for each ecotype (REF?). \comment{As such, sampling targeted carcasses found in each of these 
...
microhaplotype \citep{baetscher2018microhaplotypes} markers in California Chinook salmon.
...
The existing SNPtype assays and the additional microhaplotype markers were
developed in the absence of a well-assembled genome for Chinook salmon
\comment{(Is this true?)}. As a consequence, many of these markers had
...
the complete data set (\comment{With or without the LFAR? Which did Anthony use? I would
like to do it with the LFAR.}) 
...
\comment{I think we should also see what $K=7$ turns up}
...
the RoSA.  For this, \comment{Need to describe how I categorized
genotypes}. 
...
retained \comment{106} of those in the panel.
...
only the 2 LFAR markers we actually use. 3) consider renaming the E2 markers since
winter-run-greb1L-diagnostic is confusing...actually, I would consider removing those
entirely---we can introduce them in the subRoSA paper. We don't use them in this paper and I don't
really want to do the explanation of them again.}).
...
2.68 (RRF), with those two populations also showing the most extreme values for the
standardized fraction of polymorphic markers at 0.9 and 0.98, respectively.
...
was 0.412 (SmRF) and for observed heterozygosity, 0.412 (SmRF).
...
11 or 20 replicates in the major mode (Fig.~\ref{fig:struct}). Clustering solutions in the minor
...
groups of fish in California.  The results of the analysis are summarized
