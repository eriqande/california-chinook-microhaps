...
\end{figure}
%%%%%%%%%%%%%%%
Collections in each location were separate into fish from the different run-timing ecotypes according
to a variety of criteria.  (We need to talk about how these were split into spring run, fall-run, winter 
run, etc.
...
In order to type these markers using amplicon sequencing, we added Illumina sequencing
primers following the protocol in \citep{campbell2015genotyping}.
...
samples. The primer pairs that amplified sequence that reliably mapped back to the association- 
peak region were then typed
