...
ckmr_simQ <- function(C, unlinked = FALSE) {
  simulate_Qij(
    C,
    reps = 5e4,
...
Q_link <- ckmr_simQ(C)
Q_unl <- ckmr_simQ(C, unlinked = TRUE)

# get FPRs and FNRs for a variety of relationships.

# first for resolving unrelated individuals from PO and FS
mc_sample_simple(
  Q = Q_link,
  nu = c("PO", "FS"),
  de = "U",
